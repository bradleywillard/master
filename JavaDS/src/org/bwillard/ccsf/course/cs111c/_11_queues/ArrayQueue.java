...
import org.bwillard.ccsf.course.cs111c.labE_stacks_and_queues.NoDupsDePrioritizeQueueInterface;
 ...
 * 
 */
public class ArrayQueue<T> implements NoDupsDePrioritizeQueueInterface<T> {
	...
	public void display() {
   	...
   	public void splice(ArrayQueue<T> anotherQueue) {
   		if(anotherQueue != null && !anotherQueue.isEmpty()) {
   			int index = anotherQueue.frontIndex;
   			while(index != anotherQueue.backIndex + 1 % anotherQueue.queue.length) {
	   			T data = anotherQueue.queue[index];
	   			enqueue(data);
